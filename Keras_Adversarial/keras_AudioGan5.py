...
discriminator.add(Convolution1D(4, 256, input_dim=4, subsample_length = 16))
discriminator.add(LeakyReLU())
discriminator.add(Dropout(0.5))
...
generator.add(Convolution1D(4, 256,border_mode='same'))
generator.add(LeakyReLU())
#Use Dropouts in G in both train and test phase
generator.add(Dropout(0.5))
generator.add(BatchNormalization())
...
#   print "shape of label", y_discriminator.shape
    ...
    while 1:    
        ...
        print 'G:', loss_G
        subiteration +=1 
        print "subiteration: ", subiteration
        if loss_G < 2 or subiteration < 5:
            break
